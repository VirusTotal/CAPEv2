...
class HandlerContext:
    ...
    def copy_to_fd(self, fd, max_size=None, hasher=None):
        if max_size:
            ...
            hasher_to_update = None
        else:
            hasher_to_update = hasher
        buf = self.drain_buffer()
        if buf:
            fd.write(buf)
            if hasher_to_update:
                hasher_to_update.update(buf)
        while True:
            buf = self.read()
            if buf == b"":
                break
...
# Per-task locks and SHA256 deduplication maps
_task_locks_lock = Lock()
_task_file_locks = {}
    ...
    def handle(self):
        ...
        if file_path.endswith("_script.log"):
            ...
            try:
                ...
                log.debug("Task #%s: Failed to upload script log %s due to '%s'", self.task_id, rel_dump_path, e)
            return
